...
Adds the `expertflyer` skill: check seat availability in a named cabin, check fare-class inventory for an upgrade certificate, and create a seat alert. Verified end to end against the live account — seat alert 5736636 was created for DL2957 ATL-YYZ Comfort+ (aisle or window) and confirmed in the account.

Every alert request is **check first, alert only if absent**. An alert for something already bookable is worse than useless — it delays the booking while the operator waits for an email describing space they could have taken on the spot. `create-alert.py` also refuses to duplicate an active alert for the same flight and cabin unless `--force` is passed.

Read the structured payload, never the rendered page. Each results page ships its data as JSON in the RSC payload: `seatMap` gives every seat a `status` plus its own `isWindow`/`isAisle`/`isMiddle` flags, and `bookingClassAvailability` gives per-flight fare buckets. Text scraping the availability grid pairs an aircraft code with the AM/PM of a departure time and invents flights like `PM787`, and it missed KL642 entirely. Parsing uses a real JSON decoder because hand-rolled brace counting desynchronises on punctuation inside an alert name.

That structure also settles the seat question that started this. The rendered legend has two orthogonal axes — state (available / occupied / blocked) and decoration (wing, exit row, paid, premium, accessible) — and an early pass read grey wing shading as unavailable, reporting a full Comfort+ cabin on DL2957 while 13B and 14B were open on delta.com. The payload states `status` outright, so the ambiguity disappears; `seat_available()` still raises on an unrecognised status rather than guessing in either direction.

Cabin resolution is explicit: premium economy is Delta's **Premium Select** (`A`), a different cabin from Comfort+ (`W`). `cabin_code()` raises on an unrecognised cabin rather than defaulting to economy. A cabin the aircraft does not have yields an empty seat map that reads identically to a full one, so `check-seats.py` reports `cabin_present` and never recommends watching a cabin that can never open. A middle seat is searchable but not alertable — ExpertFlyer has no middle-seat criterion — so `criterion_values()` fails loudly instead of registering a different one.

Alert criteria bind by the checkbox `value` attribute (`AISLE`, `WINDOW`, `ANY`, `EXIT`, `TWO_TOGETHER`); index ordering and label text both fail because ids repeat across hidden tab panels and the label sits in a nested div. The `<label for=...>` is clicked rather than the input, since React ignores a programmatic `.check()` — the same trap that silently serialized `excludeCodeshares=false`. Verification re-reads the account's alert objects: `/alerts` defaults to the Flight Alerts tab, which renders "No alerts found" even when seat alerts exist, and reading that text reports a successful creation as a failure.

Access needs the stealth context from `jbaruch/fifty-tabs-of-fares`: a vanilla headless browser gets HTTP 403 on every expertflyer.com request, authenticated or not, so a 403 says nothing about the session. `check-access.py` exists to separate an expired session from the bot wall. Auth is Auth0 OIDC + PKCE with a ~7-day session; OneCLI cannot carry it, since its generic-secret injection covers header, query param and URL path but not request bodies. The session is supplied as a captured `storage_state` via `EXPERTFLYER_STORAGE_STATE`, so the container holds no ExpertFlyer password.

Fare-class alert creation is not wired — `--kind fare-class` exits with an error rather than pretending. Inventory reporting works: KL642 JFK-AMS 08/31 is `Z0` while KL646 the same evening has `Z1`.
