...
### Added — ExpertFlyer skill: seat and fare-class alerts, check-before-alert (#229)

Adds the `expertflyer` skill: check seat availability in a named cabin, check fare-class inventory for an upgrade certificate, create seat and fare-class alerts, and diagnose access. Verified against the live account — seat alert 5736694 (DL2957 ATL-YYZ Comfort+, aisle or window) and fare-class alert 5736696 (KL642 JFK-AMS, Z) were both created and confirmed present, and re-running each is refused as a duplicate.

Every alert request is **check first, alert only if absent**. An alert for something already bookable is worse than useless — it delays the booking while the operator waits for an email describing space they could have taken on the spot. `create-alert.py` also refuses to duplicate an active alert of the same kind on the same flight and class; a seat alert and a fare-class alert are different watches, so one never blocks the other.

Read the structured payload, never the rendered page. Seat maps, availability and alerts arrive in RSC responses fetched after hydration, so they are neither in the page text nor in the inlined `__next_f` payload. They are collected through a route handler that fetches each response and keeps the body: listening for responses and re-reading them later is unreliable, because Playwright cannot replay a streamed body, which made the seat map intermittently unreadable. The alert list arrives split across several payloads, so it is aggregated rather than taken from the first that parses — stopping at the first missed the fare-class alerts and let the duplicate guard through. Each payload holds: `seatMap` gives every seat a `status` plus its own `isWindow`/`isAisle`/`isMiddle` flags, `bookingClassAvailability` gives per-flight fare buckets, and the account's alerts arrive as objects. Text scraping the availability grid pairs an aircraft code with the AM/PM of a departure time and invents flights like `PM787` while missing KL642 entirely. Parsing uses a real JSON decoder because hand-rolled brace counting desynchronises on punctuation inside an alert name.

That structure also settles the seat question that started this. The rendered legend has two orthogonal axes — state (available / occupied / blocked) and decoration (wing, exit row, paid, premium, accessible) — and an early pass read grey wing shading as unavailable, reporting a full Comfort+ cabin on DL2957 while 13B and 14B were open on delta.com. The payload states `status` outright; `seat_available()` still raises on an unrecognised status rather than guessing.

Cabin resolution is explicit: premium economy is Delta's **Premium Select** (`A`), a different cabin from Comfort+ (`W`), and `cabin_code()` raises rather than defaulting to economy. A cabin the aircraft lacks yields an empty seat map that reads identically to a full one, so `check-seats.py` reports `cabin_present` and never recommends watching a cabin that can never open. A middle seat is searchable but not alertable — ExpertFlyer has no middle criterion — so `criterion_values()` fails loudly instead of registering a different one.

Form handling is bound to stable attributes, learned the hard way. Seat criteria bind by checkbox `value` (`AISLE`, `WINDOW`, ...) because ids repeat across hidden tab panels and the label text sits in a nested div; the `<label for=...>` is clicked because React ignores a programmatic `.check()` — the same trap that silently serialized `excludeCodeshares=false`. The fare-class modal is matched by its own title, `Create Flight Alert (<number>)`, rather than by row index. Its Class Code is a plain text input, not an autocomplete: pressing Enter there submits the modal, which created two alerts behind the verification step's back before it was caught. Verification re-reads the account's alert objects, because `/alerts` defaults to the Flight Alerts tab and renders "No alerts found" even when seat alerts exist — reading that text reports a successful creation as a failure.

Access needs the stealth context from `jbaruch/fifty-tabs-of-fares`: a vanilla headless browser gets HTTP 403 on every expertflyer.com request, authenticated or not, so a 403 says nothing about the session. Auth is Auth0 OIDC + PKCE with a ~7-day session. With `EXPERTFLYER_EMAIL` / `EXPERTFLYER_PASSWORD` set the skill logs in itself and retries once when a session lapses, so expiry stops being a weekly manual re-capture that always lapses mid-trip; without them it still runs from a captured `storage_state` and reports an actionable auth error. OneCLI cannot carry this credential — its generic-secret injection covers header, query param and URL path, not request bodies — so the password reaches the container as an environment variable or the session is captured by hand.
