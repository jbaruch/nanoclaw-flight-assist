...
That block's parse now lives in `travel-core/addresses.py`, shared with `drive-engine/home_address.py` rather than copied beside it. The two consumers disagree about what absence means, which is the interesting half: a missing `current_home` still raises, because a guessed drive origin mis-times every leg, while a missing `home_metro` just means suppress nothing.
