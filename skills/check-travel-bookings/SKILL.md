...
Relay the issue verbatim. The operator's `drive` / `fly` reply is handled by `Skill(skill: "tessl__drive-engine")` Step 2; either answer clears the line. An expired verdict raises no gap.
