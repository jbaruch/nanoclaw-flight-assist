...
| [expertflyer](skills/expertflyer/SKILL.md) | Action router over the operator's ExpertFlyer account: check seat availability in a named cabin (premium economy is Premium Select `A`, distinct from Comfort+ `W`), check fare-class / upgrade-certificate inventory (Z on a SkyTeam partner), and create seat or fare-class alerts. Every alert request is check-first — an already-open seat or a non-zero bucket is reported and no alert is created. Seat-map reading, cabin codes and the parameterized results URLs live in [references/web-contract.md](skills/expertflyer/references/web-contract.md). |
